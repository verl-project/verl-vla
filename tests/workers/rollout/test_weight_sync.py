...
class MockCheckpointEngine:
    ...
    async def send_weights(self, weights):
        ...
        print(f"    [Mock] send_weights called with generator")
        self.received_weights = []
    ...
    async def receive_weights(self):
        ...
        print(f"    [Mock] receive_weights called")
        for name, param in self.received_weights:
            yield name, param
        print(f"    [Mock] receive_weights completed")
...
async def test_real_update_weights(HFRolloutClass):
    ...
    try:
        ...
        print(f"    ✓ critic_backend.target_network.weight: correctly filtered (assert passed)")
...
async def test_send_receive_weights_interface():
    ...
    try:
        ...
        for name in source_weights:
            assert name in received_weights, \
                f"Assertion failed: Weight '{name}' not received"
